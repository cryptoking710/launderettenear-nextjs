...
function parseOpeningHours(hoursString: string): Record<string, string> {
  ...
  if (hoursString.includes("Collection/Delivery Only")) {
    const cleanedString = hoursString.replace("Collection/Delivery Only", "By Appointment").trim();
    const allDay = cleanedString || "By Appointment";
    return {
      monday: allDay,
      tuesday: allDay,
      wednesday: allDay,
      thursday: allDay,
      friday: allDay,
      saturday: hoursString.includes("Mon-Sat") ? allDay : "Closed",
      sunday: hoursString.includes("Mon-Sat") ? "Closed" : allDay
    };
  }

  if (hoursString.includes("Self-Service:")) {
    const selfServiceMatch = hoursString.match(/Self-Service:\s*([^,]+)/);
    if (selfServiceMatch) {
      const times = selfServiceMatch[1].trim();
      return {
        monday: times,
        tuesday: times,
        wednesday: times,
...
async function importLaunderettes() {
  console.log('🌱 Importing Oxford, Cambridge & Peterborough launderettes (26 locations)...\n');
  
  const jsonPath = path.join(__dirname, '..', 'sample-data', 'oxford-cambridge-peterborough.json');
